...
class TestLoad:
  ...
  def test_returns_credentials_when_file_exists(self, tmp_path: Path) -> None:
    self.GivenCredentialsFile(tmp_path, access_token='acc', refresh_token='ref', player_id=42)
    result = self.WhenLoaded()
    self.ThenResultIs(Credentials(access_token='acc', refresh_token='ref', player_id=42), result)
  ...
  def GivenCredentialsFile(self, tmp_path: Path, access_token: str, refresh_token: str, player_id: int | None) -> None:
    path = tmp_path / 'creds.json'
    path.write_text(json.dumps({'access_token': access_token, 'refresh_token': refresh_token, 'player_id': player_id}))
  ...
  def ThenResultIs(self, expected: Credentials | None, result: Credentials | None) -> None:
