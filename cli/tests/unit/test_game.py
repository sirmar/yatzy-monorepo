...
class TestRun:
  ...
  def test_my_turn_calls_run_my_turn(self) -> None:
    ...
    active_state = {'status': 'active', 'current_player_id': 1, 'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}
  ...
  def test_other_players_turn_calls_wait(self) -> None:
    ...
    active_state = {'status': 'active', 'current_player_id': 2, 'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}

    api.get_game_state.return_value = active_state
    api.get_scorecard.return_value = make_scorecard()
    api.get_scoreboard.return_value = [{'player_id': 1, 'total': 0}]

    with patch.object(session, '_wait_for_my_turn', return_value=(True, active_state)) as mock_wait, \
...
class TestRunMyTurn:
  ...
  def test_abort_game_returns_true(self) -> None:
    ...
    state = {'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}
    result = self._run_my_turn_with_action(session, api, state, lambda b: b['x'](), is_creator=True)
...
class TestWaitForMyTurn:
  def test_quit_returns_true_with_current_state(self) -> None:
    session, api = make_session(player_id=1)
    state = {'status': 'active', 'current_player_id': 2, 'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}
    api.get_scorecard.return_value = make_scorecard()
    ...
    with patch('yatzy.game.app') as mock_app, \
      ...
      quit_flag, returned_state = session._wait_for_my_turn(state, [1, 2], {1: 'Me', 2: 'Other'})
  ...
  def test_sse_event_updates_state_and_signals_done_when_my_turn(self) -> None:
    session, api = make_session(player_id=1)
    initial_state = {'status': 'active', 'current_player_id': 2, 'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}
    my_turn_state = {'status': 'active', 'current_player_id': 1, 'dice': [], 'rolls_remaining': 3, 'saved_rolls': 0}
    ...
    with patch('yatzy.game.app') as mock_app, \
      ...
      def run() -> None:
        result.append(session._wait_for_my_turn(initial_state, [1, 2], {1: 'Me', 2: 'Other'}))
        finished.set()
