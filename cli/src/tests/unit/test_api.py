...
def make_client(
  access_token: str = 'token', refresh_token: str = 'refresh'
) -> tuple[ApiClient, MagicMock]:
...
class TestRollDice:
  ...
  def GivenRollEndpointReturns(self, game_id: int, data: dict) -> None:
    self._route = respx.post(f'http://test/games/{game_id}/roll').mock(
      return_value=httpx.Response(200, json=data)
    )
...
class TestScoreCategory:
  ...
  def GivenScorecardEndpointReturns(
    self, game_id: int, player_id: int, data: dict
  ) -> None:
    self._route = respx.put(
      f'http://test/games/{game_id}/players/{player_id}/scorecard'
