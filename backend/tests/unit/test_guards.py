...
class TestAssertGameActive:
  ...
  def test_raises_409_when_finished(self):
    self.ThenRaises409(lambda: assert_game_active(make_game(status=GameStatus.FINISHED)))
...
class TestAssertGameInLobby:
  ...
  def test_raises_409_when_active(self):
    self.ThenRaises409(lambda: assert_game_in_lobby(make_game(status=GameStatus.ACTIVE)))

  def test_raises_409_when_finished(self):
    self.ThenRaises409(lambda: assert_game_in_lobby(make_game(status=GameStatus.FINISHED)))
