...
async def test_score_returns_200(client: AsyncClient):
  ...
  sc = await Scorecard(client).score(game.id, player.id, 'chance', token=player.token)
...
async def test_score_game_not_found_returns_404(client: AsyncClient, make_token):
  ...
  sc = await Scorecard(client).score(999, player.id, 'chance', token=player.token)
...
async def test_score_player_not_in_game_returns_404(client: AsyncClient, make_token):
  ...
  sc = await Scorecard(client).score(game.id, other.id, 'chance', token=other.token)
  sc.assert_status(404).assert_has_detail()
...
async def test_score_not_your_turn_returns_403(client: AsyncClient):
  ...
  sc = await Scorecard(client).score(game.id, p2.id, 'chance', token=p2.token)
...
async def test_score_advances_turn_to_next_player(client: AsyncClient):
  p1, p2, game = await active_game_two_players(client)
  await game.roll(game.id, p1.id)
  await Scorecard(client).score(game.id, p1.id, 'chance', token=p1.token)
...
async def test_score_wraps_turn_back_to_first_player(client: AsyncClient):
  ...
  await Scorecard(client).score(game.id, p2.id, 'chance', token=p2.token)
  state = await Game(client).state(game.id)
