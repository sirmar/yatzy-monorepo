...
async def test_finished_game_appears_in_total(client: AsyncClient):
  ...
  assert entry['standard'] == 1
  assert entry['sequential'] == 0


async def test_standard_game_appears_in_standard_sort(client: AsyncClient):
  player, _ = await _finish_one_player_game(client, mode='standard')

  response = await client.get('/games-played-leaderboard', params={'sort_by': 'standard'})
  assert response.status_code == 200
  entries = response.json()
  assert len(entries) == 1
  assert entries[0]['standard'] == 1
  assert entries[0]['sequential'] == 0


async def test_sequential_game_appears_in_sequential_sort(client: AsyncClient):
  player, _ = await _finish_one_player_game(client, mode='sequential')

  response = await client.get('/games-played-leaderboard', params={'sort_by': 'sequential'})
  assert response.status_code == 200
  entries = response.json()
  assert len(entries) == 1
  assert entries[0]['sequential'] == 1
  assert entries[0]['standard'] == 0
...
async def test_results_ordered_by_sort_field_descending(client: AsyncClient):
  ...
  for _ in range(2):
    game = await Game(client).create(p1.id, mode='standard', token=p1.token)
    await game.start(game.id, p1.id)
    for _ in range(20):
      await play_turn(client, game, game.id, p1.id, p1.token)

  game = await Game(client).create(p2.id, mode='standard', token=p2.token)
...
async def test_sort_by_sequential_ranks_by_sequential_count(client: AsyncClient):
  ...
  for _ in range(2):
    game = await Game(client).create(p2.id, mode='sequential', token=p2.token)
    await game.start(game.id, p2.id)
    for _ in range(20):
      await play_turn(client, game, game.id, p2.id, p2.token)
...
async def test_capped_at_ten_entries(client: AsyncClient):
  for i in range(11):
    ...
    game = await Game(client).create(player.id, mode='standard', token=player.token)
