...
async def test_get_scorecard_returns_404_for_unknown_game(client: AsyncClient):
  player = await Player(client).create('Alice')
  sc = await Scorecard(client).get(999, player.id)
  sc.assert_status(404).assert_has_detail()


async def test_get_scorecard_returns_404_for_player_not_in_game(client: AsyncClient):
