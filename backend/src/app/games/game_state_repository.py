...
class GameStateRepository:
  ...
  async def get(self, game_id: int) -> GameState | None:
    async with await self._conn.cursor(aiomysql.DictCursor) as cursor:
      ...
      if row is None:
        return None
      status, mode, current_turn = row['status'], row['mode'], row['current_turn']
      if status == GameStatus.FINISHED:
        ...
        entries_by_player: dict[int, dict[str, int]] = {pid: {} for pid in player_ids}
        for entry_row in entry_rows:
          pid = entry_row['player_id']
          if pid in entries_by_player:
            entries_by_player[pid][entry_row['category']] = entry_row['score']
        variant = get_variant(GameMode(mode))
        final_scores = []
        for pid in player_ids:
          scores = entries_by_player[pid]
      ...
      dice = [
        Die(index=r['die_index'], value=r['value'], kept=bool(r['kept']))
        for r in dice_rows
      ]
      return GameState(
        status=status,
        mode=mode,
        current_player_id=turn_row['player_id'],
        dice=dice,
        rolls_remaining=turn_row['rolls_remaining'],
        saved_rolls=turn_row['saved_rolls'],
      )
