...
class Game:
  ...
  async def roll(
    ...
    game_id: int,
    player_id: int,
    kept_dice: list[int] | None = None,
    token: str | None = None,
  ) -> 'Game':
    body = {'player_id': player_id, 'kept_dice': kept_dice or []}
    return self._set_response(
      await self._client.post(
        f'/games/{game_id}/roll', json=body, headers=auth_headers(token or self._token)
      )
    )
  ...
  async def delete(self, game_id: int, token: str | None = None) -> 'Game':
    return self._set_response(
      await self._client.delete(
        f'/games/{game_id}', headers=auth_headers(token or self._token)
      )
    )
  ...
  async def abort(self, game_id: int, token: str | None = None) -> 'Game':
    return self._set_response(
      await self._client.post(
        f'/games/{game_id}/abort', headers=auth_headers(token or self._token)
      )
    )

  async def start(
    self, game_id: int, player_id: int, token: str | None = None
  ) -> 'Game':
    return self._set_response(
      await self._client.post(
        f'/games/{game_id}/start',
        json={'player_id': player_id},
        headers=auth_headers(token or self._token),
      )
    )
  ...
  async def create(
    self, creator_id=None, mode: str | None = None, token: str | None = None
  ) -> 'Game':
    ...
    return self._set_response(
      await self._client.post(
        '/games', json=body, headers=auth_headers(effective_token)
      )
    )
