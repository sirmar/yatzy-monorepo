...
async def test_create_yatzy_sequential_game_returns_mode(client: AsyncClient):
  ...
  game = await Game(client).create(
    player.id, mode='yatzy_sequential', token=player.token
  )
...
async def test_yatzy_sequential_scoring_options_returns_at_most_one(
  client: AsyncClient,
):
  player, game = await active_yatzy_sequential_game(client)
  await game.roll(game.id, player.id)
...
async def test_full_yatzy_game_final_score_matches_scorecard(client: AsyncClient):
  ...
  final_total = next(
    s['total'] for s in state.json['final_scores'] if s['player_id'] == player.id
  )
