...
from rl.env.yatzy_env import YatzyEnv
...
def _collect_stats(
  model: MaskablePPO, env: YatzyEnv
  ...
  for _ in range(_STAT_EPISODES):
    ...
    scores.append(
      engine.final_score(state) if state.is_done else sum(state.scores.values())
    )
  ...
  return (
    arr.mean(),
    arr.std(),
    bonus / _STAT_EPISODES * 100,
    yatzy / _STAT_EPISODES * 100,
  )
...
class QuietEvalCallback(MaskableEvalCallback):
  ...
  def _on_step(self) -> bool:
    ...
    if self.last_mean_reward != prev:
      ...
      print(
        f'  {self.num_timesteps:>10}  {self.last_mean_reward:>8.1f}  {mean:>7.1f}  {std:>6.1f}  {bonus_pct:>6.1f}%  {yatzy_pct:>6.1f}%{suffix}'
      )
    return result
...
def make_callbacks(
  checkpoint_dir: str, log_dir: str, n_envs: int, mask_yatzy: bool, prefix: str
):
...
def train(
  ...
  if resume:
    ...
    if phase1_timesteps > 0:
      ...
      print(
        f'  {"timesteps":>10}  {"reward":>8}  {"score":>7}  {"std":>6}  {"bonus":>6}  {"yatzy":>6}'
      )
      model.learn(
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    '--resume', type=str, default=None, help='Resume phase 2 from checkpoint'
  )
