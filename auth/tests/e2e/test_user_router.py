...
async def register_and_verify(client: AsyncClient, email: str = 'user@example.com', password: str = 'password123') -> dict:
...
class TestForgotPassword:
  ...
  async def test_always_returns_200_for_unknown_email(self, client: AsyncClient):
    response = await client.post('/forgot-password', json={'email': 'nobody@example.com'})
    assert response.status_code == 200
...
class TestResetPassword:
  async def test_resets_password_successfully(self, client: AsyncClient):
    ...
    token = token_response.json()['token']
    response = await client.post('/reset-password', json={'token': token, 'new_password': 'newpassword456'})
  ...
  async def test_revokes_sessions_on_reset(self, client: AsyncClient):
    ...
    token_response = await client.get('/dev/reset-token?email=user@example.com')
    token = token_response.json()['token']
    await client.post('/reset-password', json={'token': token, 'new_password': 'newpassword456'})
    refresh_response = await client.post('/refresh', json={'refresh_token': tokens['refresh_token']})
    assert refresh_response.status_code == 401

  async def test_rejects_invalid_token(self, client: AsyncClient):
    response = await client.post('/reset-password', json={'token': 'invalid', 'new_password': 'newpassword456'})
    assert response.status_code == 400
  ...
  async def test_rejects_short_new_password(self, client: AsyncClient):
    ...
    token_response = await client.get('/dev/reset-token?email=user@example.com')
    token = token_response.json()['token']
    response = await client.post('/reset-password', json={'token': token, 'new_password': 'short'})
    assert response.status_code == 422
  ...
  async def test_rejects_reused_token(self, client: AsyncClient):
    ...
    token = token_response.json()['token']
    await client.post('/reset-password', json={'token': token, 'new_password': 'newpassword456'})
    response = await client.post('/reset-password', json={'token': token, 'new_password': 'anotherpassword'})
    assert response.status_code == 400
...
class TestDeleteAccount:
  ...
  async def test_revokes_refresh_tokens_on_delete(self, client: AsyncClient):
    reg = (await register_and_verify(client)).json()
    await client.delete('/me', headers={'Authorization': f'Bearer {reg["access_token"]}'})
    refresh_response = await client.post('/refresh', json={'refresh_token': reg['refresh_token']})
    assert refresh_response.status_code == 401
